        ...
        "questions": [
            {
                ...
                "solution": "Function Main\nDeclare Integer Array COS1103[420,3]\nDeclare Integer i\nFor i = 0 to 419\n    Loop\n        Output \"ป้อนรหัสนศ.คนที่ \" & i + 1\n        Input COS1103[i, 0]\n    Do COS1103[i, 0] < 5900000000 OR COS1103[i, 0] > 6699999999\n    Loop\n        Output \"ป้อนเพศ กด 1=ญ, 2=ช คนที่\" & i+1\n        Input COS1103[i, 1]\n    Do COS1103[i, 1] < 1 OR COS1103[i, 1] > 2\n    Loop\n        Output \"ป้อนคะแนนสอบ (เต็ม 100) คนที่ \" & i+1\n        Input COS1103[i, 2]\n    Do COS1103[i, 2] < 0 OR COS1103[i, 2] > 100\nEnd\nFor i = 0 to 419\n    Output \"คนที่ \" & i+1 & \" รหัสนศ \" & COS1103[i, 0] & \" คะแนนสอบ \" & COS1103[i, 2]\nEnd\nEnd"
            },
            {
                "problem": "คำนวณหาพื้นที่สี่เหลี่ยมผืนผ้าตามจำนวนที่ผู้ใช้ต้องการ โดยแต่ละด้านไม่เกิน 100 เมตร",
                "solution": "Function Main\nDeclare Real width, length, Area, confirm, countArea\nAssign confirm = 1\nAssign countArea = 0\nWhile confirm = 1\n    Do\n        Output \"ป้อนความกว้าง (1-100 m.)\"\n        Input width\n    Loop width < 1 OR width > 100\n    Do\n        Output \"ป้อนความยาว (1-100 m.)\"\n        Input length\n    Loop length < 1 OR length > 100\n    Assign Area = width * length\n    Assign countArea = countArea + 1\n    Output \"พื้นที่ = \" & Area\n    Do\n        Output \"กด 1 ทำต่อ หรือ กด 2 จบการทำงาน\"\n        Input confirm\n    Loop confirm < 1 OR confirm > 2\nEnd\nOutput \"หาพท.ไปทั้งหมด = \" & countArea & \" พื้นที่\"\nEnd"
            },
            {
                "problem": "แปลงหน่วยวัดจากหน่วยเมตรไปเป็นหน่วยนิ้ว (12 นิ้ว = 30 ซม. -> 1 นิ้ว = 2.5 ซม.)",
                "solution": "Function Main\nDeclare Real meter, inch\nDo\n    Output \"ป้อนความยาวหน่วยเมตร\"\n    Input meter\nLoop meter < 0.1\nAssign inch = meter * 40\nOutput \"ความยาว = \" & meter & \" เมตร = \" & inch & \" นิ้ว\"\nEnd"
            },
            {
                "problem": "จากข้อ 1 ให้สรุปจำนวนนักศึกษาหญิง ชาย รวม, คะแนนสูงสุดและเพศ, คะแนนต่ำสุดและเพศ, และคะแนนเฉลี่ย",
                "solution": "Function Main\nDeclare Integer Array COS1103[420,3]\nDeclare Integer i, max, min, countF, countM, SexMax, SexMin\nDeclare Real AvgScore, SumScore, countAll\nDeclare String SMax, SMin\nAssign countF = 0\nAssign countM = 0\nAssign SumScore = 0\nAssign countAll = 0\nAssign max = COS1103[0, 2]\nAssign min = COS1103[0, 2]\nAssign SexMax = COS1103[0, 1]\nAssign SexMin = COS1103[0, 1]\nFor i = 0 to 419\n    Assign SumScore = SumScore + COS1103[i, 2]\n    If COS1103[i, 2] > max\n        Assign max = COS1103[i, 2]\n        Assign SexMax = COS1103[i, 1]\n    End\n    If COS1103[i, 2] < min\n        Assign min = COS1103[i, 2]\n        Assign SexMin = COS1103[i, 1]\n    End\n    If COS1103[i, 1] = 1\n        Assign countF = countF + 1\n    Else\n        Assign countM = countM + 1\n    End\n    Assign countAll = countAll + 1\nEnd\nAssign AvgScore = SumScore / countAll\nIf SexMax = 1\n    Assign SMax = \"หญิง\"\nElse\n    Assign SMax = \"ชาย\"\nEnd\nIf SexMin = 1\n    Assign SMin = \"หญิง\"\nElse\n    Assign SMin = \"ชาย\"\nEnd\nOutput \"จำนวนเพศหญิง = \" & countF & \" คน จำนวนเพศชาย = \" & countM & \" คน รวมทั้งสิ้น = \" & countAll & \" คน\"\nOutput \"คะแนนสูงสุด = \" & max & \" คะแนน เป็นเพศ \" & SMax & \" คะแนนต่ำสุด = \" & min & \" คะแนน เป็นเพศ \" & SMin\nOutput \"คะแนนเฉลี่ย = \" & AvgScore & \" คะแนน\"\nEnd"
            },
            {
                ...
                "solution": "Function Main\nDeclare Real MTotal, MElec, MWater\nDeclare Integer room, wifi, elec, water\nAssign room = 3000\nAssign wifi = 100\nLoop\n    Output \"ป้อนจำนวนหน่วยไฟฟ้า\"\n    Input elec\nDo elec < 0\nLoop\n    Output \"ป้อนจำนวนหน่วยน้ำ\"\n    Input water\nDo water < 0\nIf elec > 50\n    Assign MElec = (50 * 2.5) + (elec - 50) * 3.5\nElse\n    Assign MElec = elec * 2.5\nEnd\nAssign MWater = water * 2\nAssign MTotal = room + wifi + MElec + MWater\nOutput \"จำนวนหน่วยใช้ไฟฟ้า = \" & elec & \" หน่วย ค่าไฟฟ้า = \" & MElec & \" บาท\"\nOutput \"จำนวนหน่วยใช้น้ำ = \" & water & \" หน่วย ค่าน้ำ = \" & MWater & \" บาท\"\nOutput \"รวมค่าเช่า (ค่าห้อง, wifi, ไฟฟ้า, น้ำ) = \" & MTotal & \" บาท\"\nEnd"
            }
        ]
    },
    {
